...
TORCHLIGHT_ROOT = os.path.join(PROJECT_DIR, 'torchlight')
if TORCHLIGHT_ROOT not in sys.path:
    sys.path.insert(0, TORCHLIGHT_ROOT)
...
def get_parser():
    parser = argparse.ArgumentParser(
        description='HD-GCN training for fall detection'
    )
    parser.add_argument('--work-dir', default='./work_dir/temp')
    parser.add_argument('-model_saved_name', default='')
    parser.add_argument('--config', default='./config/fall-coco/default.yaml')
    ...
    parser.add_argument('--feeder', default='feeders.feeder_coco.Feeder')
    ...
    parser.add_argument('--shadow-test-feeder-args', action=DictAction, default=dict())

    parser.add_argument('--model', default='model.HDGCN.Model')
    ...
    parser.add_argument('--step', type=int, default=[35, 55], nargs='+')
    ...
    parser.add_argument('--num-epoch', type=int, default=65)
    parser.add_argument('--weight-decay', type=float, default=0.0004)
    parser.add_argument('--lr-decay-rate', type=float, default=0.1)
    parser.add_argument('--warm_up_epoch', type=int, default=5)

    parser.add_argument('--positive-class-id', type=int, default=None)
    parser.add_argument('--positive-source-id', type=int, default=None)
    parser.add_argument('--fall-like-seed-ids', type=int, default=[], nargs='+')
    parser.add_argument('--fall-like-source-ids', type=int, default=[], nargs='+')
    parser.add_argument('--monitored-source-ids', type=int, default=[], nargs='+')
    parser.add_argument('--retained-source-ids', type=int, default=[], nargs='+')
    parser.add_argument('--squat-source-id', type=int, default=79)
    parser.add_argument('--class-weight-rule', action=DictAction, default=dict())
    parser.add_argument('--oversample-rule', action=DictAction, default=dict())
    parser.add_argument('--hard-negative-rule', action=DictAction, default=dict())
    return parser
...
class Processor(object):
    def __init__(self, arg):
        ...
        self.source_label_names = fall_detection.load_label_names(
            os.path.join(project_dir, 'label.txt')
        )
    ...
    def _init_writers(self):
        ...
        if not self.arg.train_feeder_args.get('debug', False):
            ...
            self.train_writer = self.val_writer = SummaryWriter(
                os.path.join(self.arg.model_saved_name, 'test'),
                'test'
            )
    ...
    def _load_weights_into_model(self, weights):
        ...
        for key, value in weights.items():
            normalized[key.split('module.')[-1]] = value
    ...
    def load_optimizer(self, base_lr):
        ...
        self.print_log('using warm up, epoch: {}'.format(self.current_warm_up_epoch))

    def build_train_loader(self, dataset, hard_negative_info=None):
        sampler = None
        shuffle = True
        if self.fall_config['enabled'] and self.fall_config['oversample_rule']['enabled']:
    ...
    def train_epoch(self, epoch, stage_name, save_model=False, checkpoint_prefix='runs'):
        ...
        for data, label, _index in process:
            self.global_step += 1
            with torch.no_grad():
    ...
    def evaluate_loader(self, loader_name, loader, dataset, epoch, save_score=False, score_tag=''):
        ...
        if not is_shadow_ood and len(score):
    ...
    def mine_hard_negatives(self):
        ...
        report = {
            'count': int(len(hard_negative_info['all_indices'])),
            'fall_count': int(len(hard_negative_info['fall_indices'])),
            'fall_like_count': int(len(hard_negative_info['fall_like_indices'])),
            'indices': [int(index) for index in hard_negative_info['all_indices'].tolist()],
            'fall_indices': [int(index) for index in hard_negative_info['fall_indices'].tolist()],
            'fall_like_indices': [int(index) for index in hard_negative_info['fall_like_indices'].tolist()],
        }
    ...
    def run_stage1(self):
        ...
        for epoch in range(self.arg.start_epoch, self.arg.num_epoch):
            ...
            self.train_epoch(epoch, 'stage1', save_model=save_model, checkpoint_prefix='runs')
    ...
    def run_stage2(self):
        ...
        for epoch in range(fine_tune_epochs):
            self.train_epoch(epoch, 'stage2', save_model=True, checkpoint_prefix='stage2-runs')
            eval_results = self.eval(epoch, save_score=False, loader_names=eval_loader_names)
        ...
        return final_eval_results['test'], final_report
    ...
    def summarize_training(self, final_report):
        ...
        if final_report is not None:
            ...
            self.print_log(
                ...
                'normal->fall={:.4f}, normal->fall-like={:.4f}, fall-like->fall={:.4f}, '
                'squat->fall={:.4f}'.format(
                    metrics['fall_precision'],
                    metrics['fall_recall'],
                    metrics['normal_to_fall_rate'],
                    metrics['normal_to_fall_like_rate'],
