...
OUTPUT_PATH = r"D:\Webdownload\result\S4-P4-F2-FALL-1_native_hdgcn.mp4"
HDGCN_CHECKPOINT = r"D:\Server_download\HD-GCN_best_stage1.pt"

YOLO_POSE_WEIGHTS = os.path.join(os.path.dirname(PROJECT_ROOT), "yolo26x-pose.pt")
...
def build_model(num_class):
    ...
    state_dict = load_checkpoint_weights(HDGCN_CHECKPOINT)
...
def run_hdgcn_window(model, window_data, runtime_config):
    num_person = 1
...
def main():
    ...
    print("Checkpoint:", HDGCN_CHECKPOINT)
    ...
    print("Step 2: HD-GCN action recognition...")
    ...
    for track_id, track_data in tracks.items():
        ...
        for start_idx in range(0, max(1, total_len - WINDOW_FRAMES + STRIDE_FRAMES), STRIDE_FRAMES):
            ...
            result = run_hdgcn_window(action_model, window_data, runtime_config)
