...
def _pose_shape_distance(track, det, keypoint_score_thr):
    ...
    valid = (track_scores >= keypoint_score_thr) & (det_scores >= keypoint_score_thr)
    if np.count_nonzero(valid) < 4:
        return None

    track_scale = max(1.0, track["scale"])
    det_scale = max(1.0, det["scale"])
    track_pose = (track["keypoints"][valid] - track["center"]) / track_scale
    det_pose = (det["keypoints"][valid] - det["center"]) / det_scale
...
def _predicted_center(track, reid_mode):
    ...
    steps = min(max(1, missed + 1), max_steps)
...
def _candidate_cost(track, det, dist_thr, max_missed, keypoint_score_thr, reid_mode):
    pred_center = _predicted_center(track, reid_mode)
    center_dist = float(np.linalg.norm(det["center"] - pred_center))
...
def _greedy_match(track_ids, det_indices, tracks_state, detections, dist_thr, max_missed, keypoint_score_thr, reid_mode):
    ...
    for track_id in track_ids:
        ...
        for det_index in det_indices:
            cost = _candidate_cost(track, detections[det_index], dist_thr, max_missed, keypoint_score_thr, reid_mode)
            if cost is not None:
...
def _create_track(track_id, det, frame_idx, min_hits):
    return {
        "center": det["center"],
...
def simple_tracking(
    ...
    """Track people with motion prediction, IoU, pose shape, and lost-track re-ID.

    The return format matches the original project helper:
    clean_pose_results, tracks
    """
    ...
    for frame_idx, persons in enumerate(pose_results_list):
        ...
        for track_id in list(tracks_state.keys()):
            ...
            track["missed"] += 1
            track["hits"] = 0
            drift_steps = min(track["missed"], 3)
            track["center"] = track["center"] + track["velocity"] * float(drift_steps) * 0.25
        ...
        for det_index in sorted(unmatched_dets):
            ...
            track_id = next_id
            next_id += 1
            tracks_state[track_id] = _create_track(track_id, det, frame_idx, min_hits)
            if tracks_state[track_id]["confirmed"]:
                det["person"]["track_id"] = track_id
                _append_observation(output_tracks, track_id, frame_idx, det["person"])
    ...
    print(
        "Tracking done. Valid IDs: {}, reconnected: {}, expired: {}, alive: {}".format(
